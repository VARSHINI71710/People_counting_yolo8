...
# Store trajectory points and colors for each person
trajectories = {}     # {track_id: [(x1,y1), (x2,y2), ...]}
trajectory_colors = {}  # {track_id: (B,G,R)}
...
for result in tracker:
            ...
            if cls == 0:  # person
                ...
                if track_id is not None:
                    if track_id not in trajectories:
                        ...
                        trajectory_colors[track_id] = (random.randint(0, 255),
                ...
                if track_id in trajectories and len(trajectories[track_id]) > 1:
                    for i in range(1, len(trajectories[track_id])):
                        cv2.line(frame,
                                 ...
                                 trajectory_colors[track_id], 2)

                # Draw center marker (small circle)
                cv2.circle(frame, (cx, cy), 5, trajectory_colors[track_id], -1)
    ...
    # Display video
    cv2.imshow("YOLO People Counter with Trajectories", frame)
    if cv2.waitKey(30) & 0xFF == ord("q"):  # slower playback
        break
